...
import { client } from "../../../sanity/lib/client";
import { toast } from "react-toastify";
import { RedirectToSignUp, useUser } from "@clerk/nextjs"; // Clerk import
...
function CheckoutPage() {
  const { isSignedIn, user } = useUser(); // Fetching user info here
  ...
  const createOrder = async () => {
    ...
    const newOrder = {
      ...
      status: "Pending",
      paymentId: uuidv4(),
      orderId: user.id,  // Use Clerk user ID here
      TotalPrice: totalPrice,
      product: items.map((item) => ({
        _type: "reference",
        _ref: item._id,
